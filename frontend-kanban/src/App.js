...
function App() {
  ...
  useEffect(() => {
    ...
    if (dateFilter !== 'all') {
      ...
      else if (dateFilter === '7days') startDate.setDate(now.getDate() - 7);
      else if (dateFilter === '15days') startDate.setDate(now.getDate() - 15);
      else if (dateFilter === '30days') startDate.setDate(now.getDate() - 30);
      else if (dateFilter === '60days') startDate.setDate(now.getDate() - 60);
  ...
  const onDragEnd = (result) => {
    ...
    if (activeView === 'funnel') {
      axios.post(`${API_URL}/funnel/stage`, { conversationId, stage: destination.droppableId })
          .catch(err => console.error("Erro ao atualizar estágio:", err));
      ...
      axios.post(`${API_URL}/conversations/${conversationId}/labels`, { labels: newLabels })
        .catch(err => console.error("Erro ao atualizar etiquetas:", err));
