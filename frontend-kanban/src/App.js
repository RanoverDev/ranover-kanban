...
import Modal from './components/Modal'; // Importa nosso novo componente
...
function App() {
  ...
  const [modalConfig, setModalConfig] = useState({ title: '', content: null, action: null });
  ...
  const handleAddCard = (columnId) => {
    ...
    setModalConfig({
      ...
      content: <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} className="w-full p-2 border rounded-md" autoFocus onKeyDown={(e) => e.key === 'Enter' && handleModalConfirm()} />
    });
    setIsModalOpen(true);
  };
  ...
  const handleDeleteCard = (cardId) => {
    setModalConfig({
      ...
      content: <p>Tem certeza que deseja excluir este cartão? Esta ação não pode ser desfeita.</p>
    });
    setIsModalOpen(true);
  };
  ...
  const handleEditCard = (card) => {
    ...
    setModalConfig({
      title: 'Editar Cartão',
      action: () => {
        if (!inputValue || inputValue === card.content) return;
