  ...
  const handleDeleteCard = (cardId) => {
    if (window.confirm('Tem certeza que deseja excluir este cartão?')) {
      axios.delete(`${API_URL}/cards/${cardId}`)
        .then(() => {
          fetchBoard(); // Recarrega o quadro para remover o cartão da tela
        })
        .catch(err => {
                        ...
                        {(provided) => (
                          ...
                          >
                            ...
                            <button 
                              ...
                              className="absolute top-1 right-1 w-6 h-6 rounded-full bg-slate-200 text-slate-500 hover:bg-red-500 hover:text-white opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
